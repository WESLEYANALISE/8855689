...
const patterns = {
  ...
  // Leis e artigos: Lei nº 9.504/97, Art. 5º, LC 64/90, § 1º, inciso I
  legislacao: /(?:Lei(?:\s+Complementar)?|LC|Decreto(?:-Lei)?|EC|Emenda\s+Constitucional|MP|Medida\s+Provisória|Resolução|Portaria|IN|Instrução\s+Normativa)\s*(?:n[ºo°.]?\s*)?[\d.]+(?:\/\d{2,4})?|Art(?:igo)?\.?\s*\d+[ºo°]?(?:\s*,\s*§\s*\d+[ºo°]?)?|§\s*\d+[ºo°]?|(?:inciso|alínea)\s+[IVXLCDM]+|CF(?:\/\d{2,4})?|CPP|CPC|CP|CC|CLT|CDC|CTN|CTB|ECA|Estatuto\s+(?:da\s+)?(?:Criança|Idoso|OAB|Advocacia)/gi,
  ...
  prazo: /(?:prazo\s+(?:de\s+)?)?(?:\d+)\s*(?:dias?|meses?|anos?|horas?)(?:\s+úteis)?(?:\s+(?:corridos|consecutivos))?/gi,
...
// Combinar todos os patterns em um único regex
